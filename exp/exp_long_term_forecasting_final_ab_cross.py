...
class CALayer(nn.Module):
    def __init__(self,embedding_dim=13,embedding_seq = 26,d_model=512, seq_len=24,text_embedding_dim=12, pred_len=6, prior_hidden_dim=64):
        ...
        self.prior_tf = PrivatePriorNetwork(input_dim=text_embedding_dim,
        ...
        self.tmp_linear = nn.Linear(d_model, self.text_embedding_dim)
        self.tmp_linear2 = nn.Linear(self.text_embedding_dim*2, self.text_embedding_dim)
    ...
    def forward(self, prompt_emb, preds_prompt_emb, encoder_emb):
        ...
        encoder_emb = self.tmp_linear(encoder_emb)

        ca0 = self.ca3(prompt_emb, encoder_emb, encoder_emb)
        ca0_1 = self.ca4(encoder_emb, prompt_emb, prompt_emb)
        ca0_2 = self.tmp_linear2(torch.cat((ca0, ca0_1), dim=-1))
        
        ca1 = self.ca1(preds_prompt_emb, ca0_2, ca0_2)
        ## t-sne embedding
        ca2 = self.ca2(ca0_2, preds_prompt_emb, preds_prompt_emb)
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def run_one_batch(self, batch, data_provider, text_embedding=None, preds_text_embedding=None, prior_y=None, training=False, test=False, return_align_feats=False, return_tsne_emb=False):
        ...
        if return_align_feats:
            ...
            proc_enc = self.ca_layer.tmp_linear(proc_enc)
            enc_proj = self.ca_layer.ca3.k_proj(proc_enc)   # (B, seq_len, text_dim)
