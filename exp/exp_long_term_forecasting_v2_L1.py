...
class CALayer(nn.Module):
    def __init__(self,embedding_dim=13,embedding_seq = 26,d_model=512, seq_len=24,text_embedding_dim=12, pred_len=6):
        ...
        self.pre_emb = nn.Sequential(nn.Conv1d(embedding_seq, seq_len, 3,1, 1), nn.ReLU(), nn.Dropout(0.1),nn.Linear(embedding_dim, d_model))

        
        
        
        self.coattn = CoAttention(512, self.text_embedding_dim, seq_len)
        self.ca0 = CrossAttentionLayer(query_dim=self.text_embedding_dim,
        ...
        self.ca_fusion0 = nn.Sequential(nn.Conv1d(seq_len, self.pred_len, 1, 1), nn.LeakyReLU())
        self.ca_fusion0_2 = nn.Conv1d(self.text_embedding_dim*2, 1, 1,1)
        # self.ca_fusion = nn.Sequential(nn.Conv1d(d_model, d_model*4, 1,1), nn.ReLU(), nn.Dropout(0.1))
        # self.ca_fusion2 = nn.Sequential(nn.Conv1d(d_model*4, d_model,1,1),nn.Dropout(0.1))
        # self.norm = nn.LayerNorm(1)
        # self.linear = nn.Linear(1, 1)
        self.dropout = nn.Dropout(0.1)
        self.proj_preds = nn.Conv1d(self.text_embedding_dim+self.pred_len, seq_len, 1,1)
    ...
    def forward(self, prompt_emb, preds_prompt_emb, encoder_emb, dec_inp):
        ...
        prompt_emb = torch.cat((prompt_emb[:,:12,:], torch.zeros(prompt_emb.shape[0], self.pred_len, prompt_emb.shape[2]).to(prompt_emb.device)), dim=1)
        preds_prompt_emb = torch.cat((preds_prompt_emb[:,:12,:], torch.zeros(preds_prompt_emb.shape[0], self.pred_len, preds_prompt_emb.shape[2]).to(preds_prompt_emb.device)), dim=1) #(32,18,12)
        coattn_out = self.coattn(prompt_emb, encoder_emb, encoder_emb)
        # print(coattn_out.shape) #(32,24,12)
        preds_prompt_emb = self.proj_preds(preds_prompt_emb)
        preds_prompt_emb = preds_prompt_emb + self.dropout(self.ca0(preds_prompt_emb, preds_prompt_emb, preds_prompt_emb))
        ca1 = self.ca1(preds_prompt_emb, coattn_out, coattn_out)
        ca2 = self.ca2(coattn_out, preds_prompt_emb, preds_prompt_emb)
        # print(ca1.shape,  ca2.shape) # torch.Size([32, 24, 12]) torch.Size([32, 24, 12])
        
        fus0 = self.ca_fusion0_2(self.ca_fusion0(torch.cat((ca1, ca2), dim=-1)).transpose(1,2))
        # fus = self.ca_fusion(fus0)
        # fus = self.ca_fusion2(fus).transpose(1,2)
        # print(fus0.shape, fus.shape)
        
        out = fus0.transpose(1,2)
        return out
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        elif self.args.model in ['FEDformer','Autoformer','Transformer']:
    ...
    def run_one_batch(self, batch, data_provider, text_embedding=None, preds_text_embedding=None, prior_y=None, training=False, test=False):
        ...
        if self.prompt_weight > 0:
            ...
            fus = self.ca_layer(prompt_emb, preds_prompt_emb, encoder_emb, dec_inp)
            outputs = outputs + fus
            ...
            for i, batch in enumerate(test_loader):
                ...
                if self.Doc2Vec == False:
                    ...
                    if self.pool_type == "avg":
                        ...
                        masked_emb2 = preds_prompt_emb * expanded_mask2
                        valid_counts = expanded_mask2.sum(dim=2, keepdim=True).clamp(min=1)
                        pooled_emb = masked_emb2.sum(dim=2) / valid_counts.squeeze(2)
